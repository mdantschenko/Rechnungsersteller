...
class MarkdownInvoiceArchiveReader:
    """Reads the old Markdown invoice documents into parsed value objects."""

    def read_archive(self, directory: Path) -> ArchiveReading:
        ...
        for document in sorted(directory.rglob("*.md")):
            for invoice in self.read_document(document):
                self._record_invoice(invoice, by_number, conflicts)
        return ArchiveReading(
            invoices=tuple(by_number[number] for number in sorted(by_number)),
            conflicts=tuple(conflicts),
        )

    def read_document(self, document: Path) -> Iterator[ParsedInvoice]:
        ...
        for position, anchor in enumerate(anchors):
            ...
            yield self._read_invoice(lines, anchor, end, document.name)
    ...
    def _record_invoice(
        ...
        if seen is None:
            ...
            return
        if self._describe_same_invoice(seen, invoice):
            return
        conflicts.append(
    ...
    def _read_invoice(
        self, lines: Sequence[str], anchor: int, end: int, source: str
    ) -> ParsedInvoice:
        number = int(
            self._require(
                LEGACY_INVOICE_NUMBER_PATTERN.search(lines[anchor]), source
            ).group(1)
        )
        body = "\n".join(lines[anchor:end])
        period = self._require(
            LEGACY_PERIOD_PATTERN.search(body), f"{source}, invoice {number}"
        )
        ...
        return ParsedInvoice(
            number=number,
            issued_on=self._issue_date_above(lines, anchor, number, source),
            recipient=self._recipient_above(lines, anchor, number, source),
            ...
            lines=tuple(self._read_lines(lines[anchor:end])),
            printed_total=self._read_amount(total.group(1)),
            paid_on=parse_german_date(settled.group(1)) if settled else None,
            source_file=source,
        )

    def _read_lines(self, block: Sequence[str]) -> Iterator[ParsedLine]:
        for line in block:
            ...
            yield ParsedLine(
                ...
                quantity=self._read_amount(match.group(1)),
                unit=match.group(2),
                description=match.group(3).strip(),
                unit_price=self._read_amount(match.group(5)),
                extra_printed=extra_printed,
                extra_amount=self._extra_amount(extra_printed),
                total=self._read_amount(match.group(7)),
            )
    ...
    def _recipient_above(
        self, lines: Sequence[str], anchor: int, number: int, source: str
    ) -> Address:
        collected: list[str] = []
        for index in range(anchor - 1, -1, -1):
            ...
            if (
                ...
                or LEGACY_ISSUE_DATE_PATTERN.search(line)
            ):
                continue
    ...
    def _extra_amount(self, printed: str) -> Decimal | None:
        match = LEGACY_AMOUNT_PATTERN.match(printed)
        return self._read_amount(match.group(1)) if match else None
