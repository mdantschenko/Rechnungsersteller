...
class EarningsLedger:
    """Adds up the three pots of money the pages show."""
    ...
    def monthly_earnings(
        ...
        """
        received, outstanding = self._invoices_by_month(customer)
        uninvoiced = self._uninvoiced_by_month(customer)
    ...
    def _uninvoiced_by_month(
        ...
        templates = billing_templates_by_customer(self._session, quiet_ids)
        found: dict[tuple[int, int], Decimal] = {}
        for lesson in self._session.exec(
            select(Lesson)
